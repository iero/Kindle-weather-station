...
# Added settings file by Greg Fabre, March 2015 
...
for service in root.findall('service'):
        if service.get('name') == 'netatmo' :
                _CLIENT_ID=service.find('client_id').text
                _CLIENT_SECRET=service.find('client_secret').text
                _USERNAME=service.find('username').text
                _PASSWORD=service.find('password').text
